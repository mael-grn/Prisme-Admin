...
export default function SectionElem({title, actions, loading=false, children, weight=SectionWeight.LIGHT, width=SectionWidth.AUTO}: {title?: string, loading?: boolean, actions?: ButtonProps[], children: React.ReactNode, weight?: SectionWeight, width?: SectionWidth}) {
    ...
    return (
        <div style={{width: isMobile ? SectionWidth.FULL : width, maxWidth: isMobile ? SectionWidth.FULL : width, minWidth: isMobile ? SectionWidth.FULL : width, flex: weight}} className={` max-h-full min-h-full flex flex-col`}>
            <div className={`w-full flex-1 relative p-6 bg-onBackground border-[1px] border-onBackgroundHover ${actions ? "rounded-t-xl" : "rounded-xl"} flex gap-3 flex-col`}>
            ...
            </div>
            {
                ...
                <div className={"rounded-b-xl p-3 bg-onBackground border-[1px] border-t-0 border-onBackgroundHover flex gap-3 justify-end items-center"}>
                    {
