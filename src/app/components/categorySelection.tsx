...
const slideVariants: Variants = {
    ...
    exit: {y: -8, opacity: 0, transition: {duration: 0.18, ease: [0.22, 1, 0.36, 1]}}
};
...
export default function CategorySelection({
    ...
    return (
        <div className={"w-full p-4 rounded-xl bg-background"}>
            {
                recursiveCategoryList.map((category, i) => (
                    <div key={i} className={`flex flex-col gap-3 w-full`}>
                        <div className="flex gap-4 items-center">
                            <img
                                src={`/ico/chevron-down.svg`}
                                alt={"deploy"}
                                className={`invert w-6 ${expandedCategories.includes(category.id) && "rotate-180"} cursor-pointer`}
                                onClick={() => {
                        ...
                        <AnimatePresence>
                            {
                                ...
                                <motion.div
                                    ...
                                    exit="exit"
                                    layout
                                    className={"overflow-hidden flex flex-col gap-2 pl-12 mb-4"}
                                >
                                    {
                                        ...
                                        category.subcategories.map((subcategory, j) => {
                                            ...
                                            return (
                                                <label key={j} className="flex items-center gap-4 group cursor-pointer pl-2">
                                                    <input
                                                        type="checkbox"
                                                        className="sr-only"
                                                        checked={isChecked}
          ...
          {isChecked && (
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                  ...
                  <path d="M5 13l4 4L19 7" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                                    ...
                                    <button type={"button"} className={"border-none bg-transparent md:hover:bg-onBackgroundHover active:bg-onBackgroundHover active:scale-90 rounded-lg flex pt-1 pb-1 pl-2 pr-2 justify-start items-center gap-3 w-fit"} onClick={() => onCreateSubcategoryPressedAction(category)}>
